...
import { CV_TEMPLATES_V2, type CVTemplateConfig as CVTemplateConfigV2 } from './cv-renderer-v2';
...
export const TEMPLATE_COUNTS = {
    ...
    cv: {
        free: CV_TEMPLATES_V2.filter(t => t.tier === 'free').length,
        starter: CV_TEMPLATES_V2.filter(t => t.tier === 'free' || t.tier === 'starter').length,
        pro: CV_TEMPLATES_V2.length,
    },
};

// ============================================
// CV TEMPLATES - Adapterad från CV_TEMPLATES_V2
// Konverterar V2-format till gallery-format
// ============================================
...
// Adapter: Konvertera CV_TEMPLATES_V2 till CVTemplate format för gallery
function adaptV2Template(t: CVTemplateConfigV2, index: number): CVTemplate {
    return {
        id: t.id,
        name: t.name,
        ...
        fonts: {
            heading: t.fontHeading,
            body: t.fontBody,
        },
        layout: 'sidebar', // Alla V2-templates använder sidebar-layout
        atsOptimized: t.category === 'professional' || t.category === 'minimal',
        popular: index < 5, // Första 5 är populära
    };
}

// Generera ALL_CV_TEMPLATES från V2
export const ALL_CV_TEMPLATES: CVTemplate[] = CV_TEMPLATES_V2.map(adaptV2Template);
